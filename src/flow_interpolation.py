...
def warp_flow(firstImage, secondImage, forward_flow, If, backward_flow, Ib, image_ind, EST_DIR):
    """
    ...
    firstImage = cv2.imread(firstImage, cv2.IMREAD_GRAYSCALE).astype(float)
    secondImage = cv2.imread(secondImage, cv2.IMREAD_GRAYSCALE).astype(float)
    ...
    """
    cv2.imwrite(
        os.path.join(EST_DIR, 'forward_prediction_{}.png'. format(image_ind + 1)),
        forward_prediction,
    )
    """
    ub, vb = backward_flow
    ...
    """
    cv2.imwrite(
        os.path.join(EST_DIR, 'backward_prediction_{}.png'. format(image_ind + 1)),
        backward_prediction,
    )
    """
    ...
    It = It.astype(np.int)
    cv2.imwrite(
        os.path.join(EST_DIR, 'interpolated_{}.png'. format(image_ind + 1)),
        It,
    )
